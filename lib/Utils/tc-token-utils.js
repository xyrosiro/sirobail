...
function isRegularUser(jid) {
  ...
  if ((0, _index.isJidMetaAI)(jid)) return false; // MetaAI (@bot server)
  return !!((0, _index.isPnUser)(jid) || (0, _index.isLidUser)(jid) || (0, _index.isHostedPnUser)(jid) || (0, _index.isHostedLidUser)(jid) || jid.endsWith('@c.us'));
}
...
const TC_TOKEN_INDEX_KEY = exports.TC_TOKEN_INDEX_KEY = '__index';
...
async function buildMergedTcTokenIndexWrite(keys, addedJids) {
  ...
  return {
    [TC_TOKEN_INDEX_KEY]: {
      token: Buffer.from(JSON.stringify([...merged]))
    }
  };
}
...
async function resolveTcTokenJid(jid, getLIDForPN) {
  if ((0, _index.isLidUser)(jid)) return jid;
...
async function storeTcTokensFromIqResult({
  result,
  fallbackJid,
  keys,
  getLIDForPN,
  ...
  const tokensNode = (0, _index.getBinaryNodeChild)(result, 'tokens');
  if (!tokensNode) return;
  const tokenNodes = (0, _index.getBinaryNodeChildren)(tokensNode, 'token');
  for (const tokenNode of tokenNodes) {
    if (tokenNode.attrs.type !== 'trusted_contact' || !(tokenNode.content instanceof Uint8Array)) {
      continue;
    }
    // In notifications tokenNode.attrs.jid is your own device JID, not the sender's
    const rawJid = (0, _index.jidNormalizedUser)(fallbackJid || tokenNode.attrs.jid);
