...
// ─────────────────────────────────────────────
// SIROBAIL: richResponse — keyword set untuk syntax highlighting code block
// ─────────────────────────────────────────────
const RICH_RESPONSE_CODE_KEYWORDS = new Set([
  ...
  'null', 'undefined', 'NaN', 'Infinity', 'class', 'const', 'let',
  'super', 'extends', 'export', 'import', 'yield', 'static',
  'constructor', 'of', 'async', 'await', 'get', 'set',
  'implements', 'interface', 'package', 'private', 'protected',
  'public', 'enum', 'throws', 'transient'
]);

// SIROBAIL: richResponse — tokenizer sederhana untuk code block
...
const generateWAMessageContent = async (message, options) => {
  ...
  if ('richResponse' in message) {
    // SIROBAIL: richResponse — pesan format AI/bot (text, code, table, latex, map, image)
    const {
      text: richText,
      code,
      language = 'javascript',
    ...
    if (richText) {
      sections.push({
        view_model: {
          primitive: { text: richText, __typename: 'GenAIMarkdownTextUXPrimitive' },
    ...
    const unifiedData = {
      response_id: responseId || (0, _crypto.randomUUID)(),
      sections
    };
    return _index.proto.Message.fromObject({
      messageContextInfo: {
        deviceListMetadata: {},
        deviceListMetadataVersion: 2,
        messageSecret: richSecret || (0, _crypto.randomBytes)(32)
      },
      botForwardedMessage: {
        message: {
          richResponseMessage: {
