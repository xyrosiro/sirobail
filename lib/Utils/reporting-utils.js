...
exports.shouldIncludeReportingToken = exports.getMessageReportingToken = void 0;
...
const generateMsgSecretKey = (modificationType, origMsgId, origMsgSender, modificationSender, origMsgSecret) => {
  ...
  return (0, _crypto2.hkdf)(origMsgSecret, 32, {
    info: useCaseSecret.toString('latin1')
  });
};
const extractReportingTokenContent = (data, cfg) => {
  ...
  while (i < data.length) {
    ...
    const pushSlice = end => {
      ...
      out.push({
        num: fieldNum,
        bytes: data.subarray(fieldStart, end)
      });
      i = end;
      return true;
    };
    ...
    if (wireType === WIRE.BYTES) {
      ...
      out.push({
        num: fieldNum,
        bytes: data.subarray(fieldStart, valEnd)
      });
      i = valEnd;
      continue;
    }
...
const getMessageReportingToken = async (msgProtobuf, message, key) => {
  ...
  const reportingToken = (0, _crypto.createHmac)('sha256', reportingSecret).update(content).digest().subarray(0, 16);
  return {
    tag: 'reporting',
    attrs: {},
    content: [{
