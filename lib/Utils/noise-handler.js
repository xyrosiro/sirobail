...
class TransportState {
  ...
  encrypt(plaintext) {
    ...
    this.iv[10] = c >>> 8 & 0xff;
    this.iv[11] = c & 0xff;
    return (0, _crypto.aesEncryptGCM)(plaintext, this.encKey, this.iv, EMPTY_BUFFER);
  }
  decrypt(ciphertext) {
    ...
    this.iv[10] = c >>> 8 & 0xff;
    this.iv[11] = c & 0xff;
    return (0, _crypto.aesDecryptGCM)(ciphertext, this.decKey, this.iv, EMPTY_BUFFER);
  }
}
const makeNoiseHandler = ({
  ...
  const encrypt = plaintext => {
    ...
    const result = (0, _crypto.aesEncryptGCM)(plaintext, encKey, generateIV(counter++), hash);
    authenticate(result);
    return result;
  };
  const decrypt = ciphertext => {
    ...
    const result = (0, _crypto.aesDecryptGCM)(ciphertext, decKey, generateIV(counter++), hash);
    authenticate(ciphertext);
    return result;
  };
  const localHKDF = data => {
    const key = (0, _crypto.hkdf)(Buffer.from(data), 64, {
      salt,
      info: ''
    });
  ...
  return {
    ...
    processHandshake: ({
      ...
      mixIntoKey(_crypto.Curve.sharedKey(privateKey, serverHello.ephemeral));
      const decStaticContent = decrypt(serverHello.static);
      mixIntoKey(_crypto.Curve.sharedKey(privateKey, decStaticContent));
      const certDecoded = decrypt(serverHello.payload);
      const {
        intermediate: certIntermediate,
        leaf
      } = _index.proto.CertChain.decode(certDecoded);
      // leaf
      if (!leaf?.details || !leaf?.signature) {
        throw new _boom.Boom('invalid noise leaf certificate', {
          statusCode: 400
        });
      }
      if (!certIntermediate?.details || !certIntermediate?.signature) {
        throw new _boom.Boom('invalid noise intermediate certificate', {
          statusCode: 400
        });
      }
      const details = _index.proto.CertChain.NoiseCertificate.Details.decode(certIntermediate.details);
      const {
        issuerSerial
      } = details;
      const verify = _crypto.Curve.verify(details.key, leaf.details, leaf.signature);
      const verifyIntermediate = _crypto.Curve.verify(_index2.WA_CERT_DETAILS.PUBLIC_KEY, certIntermediate.details, certIntermediate.signature);
      if (!verify) {
        throw new _boom.Boom('noise certificate signature invalid', {
          statusCode: 400
        });
      }
      if (!verifyIntermediate) {
        throw new _boom.Boom('noise intermediate certificate signature invalid', {
          statusCode: 400
        });
      }
      if (issuerSerial !== _index2.WA_CERT_DETAILS.SERIAL) {
        throw new _boom.Boom('certification match failed', {
          statusCode: 400
        });
      }
      const keyEnc = encrypt(noiseKey.public);
      mixIntoKey(_crypto.Curve.sharedKey(noiseKey.private, serverHello.ephemeral));
      return keyEnc;
    },
    encodeFrame: data => {
      if (transport) {
